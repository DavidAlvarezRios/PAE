...
/**
 * Funcio per enviar les dades als moduls
 *
...
byte TxPacket(byte bID, byte bParameterLength, byte bInstruction, byte Parametros[16])
{
    ...
    while( (UCA2STATW&UCBUSY)); //Espera fins que s'ha transmes el ultim byte
    sentit_dades_rx(); //Posem la linia de dades en Rx perque el modul Dynamixel envia resposta
    return(bPacketLength);
}

/**
 * Funció per rebre les dades dels moduls
 *
 * Dades d'entrada: cap
 *
 * Dades de sortida: RxReturn amb StatusPacket, timeout i checksum.
 */
...
void TA0_0_IRQHandler(void)
{
    ...
    count_ms += 1; //Aumentamos el contador para saber cuï¿½nto tiempo ha transcurrido en funciï¿½n del tiempo que tarde en llamarse dicha interrupciï¿½n.
    TA0CCTL0 &= ~CCIFG;
    TA0CCTL0 |= CCIE; //Habilitem la interrupciÃ³
}

/*****************************************************************************
 * GestiÃ³n de la interrupcion del timer 1
 *
 * Sin datos de entrada
 *
 * Sin datos de salida
...
void TA1_0_IRQHandler(void)
{
    ...
    count_timeout += 1; //Aumentamos el contador para saber cuï¿½nto tiempo ha transcurrido en funciï¿½n del tiempo que tarde en llamarse dicha interrupciï¿½n.
    TA1CCTL0 &= ~CCIFG;
    TA1CCTL0 |= CCIE; //Habilitem la interrupciÃ³
}
